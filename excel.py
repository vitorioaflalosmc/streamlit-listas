        ...
        if json_filename.startswith("lista1"):
            ...
            try:
                ...
                for cell in cell_ranges:
                    ...
                    cell_range.api.HorizontalAlignment = -4131  # Alinhamento à esquerda

                # Definir área de impressão
                ws.api.PageSetup.PrintArea = ws.range('A1:D1000').api.Address  # Ajuste conforme necessário

                # Salvar o novo arquivo Excel
                wb.save(novo_excel_filename)
                st.success(f"Novo arquivo Excel salvo como: {novo_excel_filename}")
            finally:
                wb.close()  # Certifique-se de que o workbook seja fechado
